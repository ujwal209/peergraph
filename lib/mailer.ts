...
export async function sendOTPEmail(email: string, code: string, type: "signup" | "recovery") {
  const subject = type === "signup" 
    ? "Peergraph Intelligence | Initialization Sequence" 
    : "Peergraph Intelligence | Recovery Sequence";

  const message = type === "signup"
    ? `Your initialization sequence code is: ${code}. This code expires in 10 minutes.`
    : `Your recovery sequence code is: ${code}. This code expires in 10 minutes.`;
  ...
  await transporter.sendMail({
    from: `"Peergraph Intelligence" <${process.env.SMTP_FROM || process.env.SMTP_USER}>`,
    to: email,
    subject: subject,
    text: message,
    html: html,
